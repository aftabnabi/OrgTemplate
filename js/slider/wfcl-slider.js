jQuery.fn.wowSlider = function (x) {
    ...
    function S(ac, Y, aa, Z) {
        var f, c, ab = 0;
        if (ac.addEventListener) {
            ac.addEventListener("touchmove", function (ad) {
                if (ab) {
                    if (Y(ad, f - ad.touches[0].pageX, c - ad.touches[0].pageY)) {
                        f = c = ab = 0
                    }
                }
                return false
            }, false);
            ac.addEventListener("touchstart", function (ad) {
                if (ad.touches.length == 1) {
                    f = ad.touches[0].pageX;
                    c = ad.touches[0].pageY;
                    ab = 1;
                    if (aa) {
                        aa(ad)
                    }
            ...
            ac.addEventListener("touchend", function (ad) {
                ab = 0;
                if (Z) {
                    Z(ad)
                }
    ...
    function e() {
        k.find(".wfcl-bullets a,.ws_thumbs a").click(function (aj) {
            W(aj, D(this).index())
        });
        if (L.length) {
            L.hover(function () {
            ...
            var ag;
            L.mousemove(function (am) {
                if (ag) {
                    return
                }
                var ao = 0.1;
                for (var al = 0; al < 2; al++) {
                    var ak = L[al ? "width" : "height"](),
                        aj = ae[al ? "width" : "height"](),
                        an = ak - aj;
                    if (an < 0) {
                        an *= Math.min(Math.max(((am[al ? "pageX" : "pageY"] - L.offset()[al ? "left" : "top"]) / ak - ao) / (1 - 2 * ao), 0), 1);
                        ae.stop(true).animate(al ? {
                            left: an
                        } : {
                            top: an
                        }, aj / 2, "easeOutCubic")
                    } else {
                        ae.css(al ? "left" : "top", al ? an / 2 : 0)
                    }
                }
            });
            L.trigger("mousemove");
            var ab, ac;
            Q(ae.get(0), function (al, ak, aj) {
                ae.css("left", Math.min(Math.max(ab - ak, L.width() - ae.width()), 0));
                ae.css("top", Math.min(Math.max(ac - aj, L.height() - ae.height()), 0));
                al.preventDefault();
                return false
            }, function (aj) {
                ab = parseFloat(ae.css("left")) || 0;
                ac = parseFloat(ae.css("top")) || 0;
                return false
            });
            var Z;
            k.find(".ws_thumbs a").each(function (aj, ak) {
                Q(ak, function (al) {
                    Z = 1
                }, 0, function (al) {
                    ag = 1;
        ...
        if (l.length) {
            ...
            var Y = af.find("IMG");
            if (Y.length) {
                var aa = D('<div class="wfcl-bulframe"/>').appendTo(ai);
                var f = D("<div/>").css({
                    width: Y.length + 1 + "00%"
                }).appendTo(D("<div/>").appendTo(aa));
                Y.appendTo(f);
                D("<span/>").appendTo(aa);
                var c = -1;
                ...
                function ad(al) {
                    ...
                    D(af.get(c)).removeClass("ws_overbull");
                    D(af.get(al)).addClass("ws_overbull");
                    aa.show();
                    var am = {
                        left: af.get(al).offsetLeft - aa.width() / 2,
                        "margin-top": af.get(al).offsetTop - af.get(0).offsetTop + "px",
                        "margin-bottom": -af.get(al).offsetTop + af.get(af.length - 1).offsetTop + "px"
                    };
                    var ak = Y.get(al);
                    var aj = {
                        left: -ak.offsetLeft + (D(ak).outerWidth(true) - D(ak).outerWidth()) / 2
                    };
                    if (c < 0) {
                        aa.css(am);
                        f.css(aj)
                        ...
                        aa.stop().animate(am, "fast");
                        f.stop().animate(aj, "fast")
                    }
                    c = al
                }
                af.hover(function () {
                ...
                ai.click(function (aj) {
                    W(aj, D(aj.target).index())
                })
            }
        }
    }
    ...
    function R(c) {
        D("A", l).each(function (f) {
            if (f == c) {
                D(this).addClass("wfcl-sellbull")
            } else {
                D(this).removeClass("wfcl-sellbull")
            }
        })
    }
    if (x.caption) {
        $caption = D("<div class='wfcl-title' style='display:none'></div>");
        k.append($caption);
...
function wowsliderPreloader(m, d) {
    var f = jQuery,
        j = d.images,
        n = [],
        q = d.logo || "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
    ...
    var e = f("<div class='wfcl-sl-logo'></div>").hide().appendTo(m);
    var s;
    var l;
